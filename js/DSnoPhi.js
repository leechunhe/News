  ...
  fetch('DSnoPhi.xml')
    ...
    .then(data => {
      ...
      for (let row of rows) {
        ...
        originalData.push({ STT, MaDV, TenDV, TongTien });
      }
  ...
  contentDiv.innerHTML = `
    ...
    <table border="1" width="100%">
      <tr style="background-color: #5EB7F8; color: white; align:center;">
        <th style="text-align: center;">STT</th><th style="text-align: center;">Mã DV</th><th  style="text-align: center;">Tên đơn vị</th><th style="text-align: center;">Tổng Tiền (VNĐ)</th>
      </tr>
      ${pageData.map(item => `
        <tr>
          <td>${item.STT}</td>
